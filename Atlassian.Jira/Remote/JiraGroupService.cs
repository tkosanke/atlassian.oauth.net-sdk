...
namespace Atlassian.Jira.Remote
{
    internal class JiraGroupService : IJiraGroupService
    {
        ...
        {
            var resource = String.Format("rest/api/latest/group/user?groupname={0}", Uri.EscapeDataString(groupname));
        ...
        {
            var resource = String.Format("rest/api/latest/group?groupname={0}", Uri.EscapeDataString(groupName));
        ...
        public async Task<IPagedQueryResult<JiraUser>> GetUsersAsync(string groupname, bool includeInactiveUsers = false, int maxResults = 50, int startAt = 0, CancellationToken token = default(CancellationToken))
        {
            var resource = String.Format(
                "rest/api/latest/group/member?groupname={0}&includeInactiveUsers={1}&startAt={2}&maxResults={3}",
        ...
        {
            var resource = String.Format("rest/api/latest/group/user?groupname={0}&username={1}",
                Uri.EscapeDataString(groupname),
